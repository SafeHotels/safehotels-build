...
const TP_MARKER   = "501240";

// Generates a search-based URL — NEVER gets a 404, works for every hotel
function bookingUrl(hotel, checkin, checkout, guests) {
  const q  = encodeURIComponent((hotel.name || '') + ', ' + (hotel.city || '') + ', UK');
  const ci = checkin  || '';
  const co = checkout || '';
  const g  = guests   || 2;
  return `https://www.booking.com/search.html?ss=${q}&aid=${BOOKING_AID}&label=safehotels-tp${TP_MARKER}&checkin=${ci}&checkout=${co}&group_adults=${g}&lang=en-gb`;
}
// ─────────────────────────────────────────────────────────────────────────────
